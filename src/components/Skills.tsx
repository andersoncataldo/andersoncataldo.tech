        ...
        <ul className="grid md:grid-cols-3 gap-8 mb-16" role="list">
          {skillGroups.map((group, index) => (
            ...
            >
              ...
              <h3 id={`skill-group-${index}`} className="text-2xl font-bold mb-8 text-apple-text tracking-tight">{group.title}</h3>
              <ul className="flex flex-wrap gap-3" aria-labelledby={`skill-group-${index}`}>
                {group.skills.map((skill) => (
                  <li
                    key={skill}
                    className="px-4 py-2 bg-apple-bg text-apple-text text-sm font-semibold rounded-xl border border-apple"
                  >
                    {skill}
                  </li>
                ))}
        ...
        >
          <div className="flex flex-col lg:flex-row items-center justify-between gap-12">
            <div className="flex items-center gap-6">
              ...
              <div>
                <h4 id="soft-skills-title" className="text-2xl font-bold text-apple-text">Mindset Profissional</h4>
            ...
            <ul className="flex flex-wrap justify-center gap-4" aria-labelledby="soft-skills-title">
              {softSkills.map((skill) => (
                <li key={skill} className="text-sm font-bold text-apple-text px-6 py-3 bg-apple-bg rounded-2xl border border-apple shadow-sm">
                  {skill}
                </li>
              ))}
            </ul>
